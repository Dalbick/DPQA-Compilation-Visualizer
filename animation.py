...
class Inst(ABC):
    ...
    def write_code(
        ...
        curr = {}
        curr["qubits"] = [
            {"id": q.id, "x": q.x, "y": q.y, "array": q.array, "c": q.c, "r": q.r}
            for q in qubit_objs
        ]
        curr["cols"] = [{"id": c.id, "active": c.active, "x": c.x} for c in col_objs]
        curr["rows"] = [{"id": r.id, "active": r.active, "y": r.y} for r in row_objs]
    ...
    def __init__(
        ...
        super().__init__(
            ...
            reduced_keys=[
                ...
                "y_high",
                "c_high",
                "r_high",  # TODO: add single-qubit gate parameters
            ],
        )
        for k, v in data.items():
            self.code[k] = v
...
class Move(Inst):
    ...
    def verify(
        ...
        if not (a == b and a == c):
            raise ValueError(
                f"{self.name}: col arguments invalid" f" {a} idx, {b} begin, {c} end."
            )
        a = len(row_idx)
        b = len(row_begin)
        c = len(row_end)
        if not (a == b and a == c):
            raise ValueError(
                f"{self.name}: row arguments invalid" f" {a} idx, {b} begin, {c} end."
            )
        ...
        for i, moving_col_id in enumerate(col_idx):
            if moving_col_id not in activated_col_idx:
                raise ValueError(
                    f"{self.name}: col {moving_col_id} to move" f" is not activated."
                )
        ...
        for i, moving_row_id in enumerate(row_idx):
            if moving_row_id not in activated_row_idx:
                raise ValueError(
                    f"{self.name}: row {moving_row_id} to move " f"is not activated."
                )
    ...
    def emit(self):
        code = {"type": self.type}
        code["cols"] = [
            {k: v for k, v in tmp.items() if k in ["id", "shift"]}
            for tmp in self.code["cols"]
        ]
        code["rows"] = [
            {k: v for k, v in tmp.items() if k in ["id", "shift"]}
            for tmp in self.code["rows"]
        ]
        return (code,)
...
class Activate(Inst):
    ...
    def verify(
        ...
        b = len(col_xs)
        if a != b:
            raise ValueError(f"{self.name}: col arguments invalid {a} idx, {b} xs")
        a = len(row_idx)
        b = len(row_ys)
        if a != b:
            raise ValueError(f"f{self.name}: row arguments invalid {a} idx, {b} ys.")

        for i in range(len(col_idx)):
            if col_objs[col_idx[i]].active:
                raise ValueError(f"{self.name}: col {col_idx[i]} already activated.")
            for j in range(col_idx[i]):
                if col_objs[j].active and col_objs[j].x > col_xs[i] - AOD_SEP:
                    raise ValueError(
            ...
            for j in range(col_idx[i] + 1, len(col_objs)):
                if col_objs[j].active and col_objs[j].x - AOD_SEP < col_xs[i]:
                    raise ValueError(
                        f"{self.name}: col {j} at x={col_objs[j].x} is "
        ...
        for i in range(len(row_idx)):
            if row_objs[row_idx[i]].active:
                raise ValueError(f"{self.name}: row {row_idx[i]} already activated.")
            for j in range(row_idx[i]):
                if row_objs[j].active and row_objs[j].y > row_ys[i] - AOD_SEP:
                    raise ValueError(
            ...
            for j in range(row_idx[i] + 1, len(row_objs)):
                if row_objs[j].active and row_objs[j].y - AOD_SEP < row_ys[i]:
                    raise ValueError(
...
class Deactivate(Inst):
    def __init__(
        ...
        super().__init__(
            "Deactivate", prefix=prefix, stage=s, reduced_keys=["col_idx", "row_idx"]
        )
        self.verify(
            col_objs, row_objs, qubit_objs, col_idx, col_xs, row_idx, row_ys, dropoff_qs
        )
        self.operate(col_objs, row_objs, qubit_objs, col_idx, row_idx, dropoff_qs)
        super().write_code(
            col_objs,
            row_objs,
            qubit_objs,
    ...
    def verify(
        ...
        b = len(row_ys)
        if a != b:
            raise ValueError(f"{self.name}: row arguments invalid {a} idx, {b} ys.")

        for i in range(len(col_idx)):
            if not col_objs[col_idx[i]].active:
                raise ValueError(f"{self.name}: col {col_idx[i]} already dectivated.")
            for j in range(col_idx[i]):
                if col_objs[j].active and col_objs[j].x > col_xs[i] - AOD_SEP:
                    raise ValueError(
                        f"{self.name}: col {j} at x={col_objs[j].x} is "
        ...
        for i in range(len(row_idx)):
            if not row_objs[row_idx[i]].active:
                raise ValueError(f"{self.name}: row {row_idx[i]} already deactivated.")
            for j in range(row_idx[i]):
                if row_objs[j].active and row_objs[j].y > row_ys[i] - AOD_SEP:
                    raise ValueError(
                        f"{self.name}: row {j} at y={row_objs[j].y} is "
...
class Rydberg(Inst):
    def __init__(
        ...
        row_objs: Sequence[Row],
        qubit_objs: Sequence[Qubit],
        gates: Sequence[Mapping[str, int | str]],
    ):
    ...
    def verify(
        self, gates: Sequence[Mapping[str, str | int]], qubit_objs: Sequence[Qubit]
    ):
        # for g in gates:
...
class ComboInst:
    ...
    def __init__(
        ...
        self.type = type
        self.name = (
            (f"{prefix}:" if prefix else "") + type + (f"_{suffix}" if suffix else "")
        )
        self.stage = stage
        self.duration = -1
        self.insts = []
...
class SwapPair(ComboInst):
    ...
    def __init__(
        ...
        super().__init__(
            "SwapPair", prefix=prefix, suffix=f"({left_q_id},{right_q_id})", stage=s
        )
...
class CodeGen:
    ...
    def __init__(
        ...
        if not dir:
            dir = "./results/code/"
        self.code_full_file = dir + (file_name.split("/")[-1]).replace(
            ".json", "_code_full.json"
        )
    ...
    def read_compiled(self, filename: str):
        ...
        for i in range(self.n_t - 1, 0, -1):
            for q in range(self.n_q):
                self.layers[i]["qubits"][q]["a"] = self.layers[i - 1]["qubits"][q]["a"]
                self.layers[i]["qubits"][q]["c"] = self.layers[i - 1]["qubits"][q]["c"]
                self.layers[i]["qubits"][q]["r"] = self.layers[i - 1]["qubits"][q]["r"]
    ...
    def aod_from_compiled(self):
        for s, layer in enumerate(self.layers):
            if s == 0:
                continue
            layer["row"] = [{"id": i, "qs": []} for i in range(self.r_high - 0)]
            layer["col"] = [{"id": i, "qs": []} for i in range(self.c_high - 0)]
    ...
    def builder(self, no_transfer: bool):
        ...
        for s in range(1, len(self.layers)):
            ...
            self.builder_raman(s, cols, rows, qubits, program, init)
    ...
    def builder_init(
        ...
        ]  # put all qubits in the left trap
        ...
        self.builder_raman(0, cols, rows, qubits, program, init)
            ...
            for q1_id in range(q0_id + 1, self.n_q):
                ...
                if q0_a == 1 and q1_a == 1:
                    ...
                    if (q0_x, q0_y, q0_r) == (q1_x, q1_y, q1_r):
                        ...
                        if (q0_c > q1_c and qubits[q0_id].x < qubits[q1_id].x) or (
                            q0_c < q1_c and qubits[q0_id].x > qubits[q1_id].x
                        ):
                            swap_obj.add_swap_pair(cols, rows, qubits, q0_id, q1_id)
    ...
    def builder_reload(
        ...
        for row_id in range(self.r_high):
            if layer["row"][row_id]["qs"]:  # there are qubits to load
                    ...
                    if len(site_qs) == 2:
                        ...
                        if not cols[col_id_left].active:
                            ...
                            reloadRow_obj.add_col_shift(
                                id=col_id_left, begin=cols[col_id_left].x, end=lower_x
                            )
                        ...
                        if not cols[col_id_right].active:
                            ...
                            reloadRow_obj.add_col_shift(
                                id=col_id_right, begin=cols[col_id_right].x, end=upper_x
                            )
                ...
                num_rows = len(layer["y_rows_begin"][layer["row"][row_id]["y_begin"]])
                shift_down = (num_rows - layer["row"][row_id]["offset_begin"]) * AOD_SEP
                # Also shift the Cols by 1 to avoid collisions.
                col_idx = [col_id for col_id, col in enumerate(cols) if col.active]
                col_begin = [cols[col_id].x for col_id in col_idx]
                col_end = [
                    1
                    + layer["col"][col_id]["x_begin"] * X_SITE_SEP
                    + AOD_SEP * layer["col"][col_id]["offset_begin"]
                    for col_id in col_idx
                ]
                reloadRow_obj.generate_parking(
                    cols, rows, qubits, shift_down, col_idx, col_begin, col_end
                )
        ...
        for row_id in range(self.r_high):
            if rows[row_id].active:
                ...
                for site_x in range(self.x_high):
                    ...
                    for q_id, q in enumerate(layer["qubits"]):
                        if (q["x"], q["y"]) == (site_x, layer["row"][row_id]["y_end"]):
                            if qubits[q_id].array == "AOD" and q["r"] == row_id:
                    ...
                    if len(site_q_aod) == 2:
                        ...
                        offloadRow_obj.add_col_shift(
                            id=col_id_left, begin=qubits[q_id_left].x, end=lower_x
                        )
                        offloadRow_obj.add_col_shift(
                            id=col_id_right, begin=qubits[q_id_right].x, end=upper_x
                        )
                        ...
                        if site_q_slm:
                            lower_x = (
                                2 * X_SITE_SEP * site_x
                                + SITE_WIDTH
                                - qubits[site_q_slm[0]].x
                            )
                        upper_x = lower_x
                        offloadRow_obj.add_col_shift(
                            id=col_id_left, begin=qubits[q_id].x, end=lower_x
                        )
                ...
                offloadRow_obj.generate_row_shift(
                    cols, rows, qubits, layer["row"][row_id]["y_end"]
                )
                offloadRow_obj.generate_row_deactivate(cols, rows, qubits, dropoff_qs)
...
class Animator:
    ...
    def create_master_schedule(self, real_speed: bool):
        ...
        for inst in self.code:
            ...
            new_frame = frame + int((inst["duration"] + MUS_PER_FRM - 1) / MUS_PER_FRM)
    ...
    def setup_canvas(self, scaling_factor: int):
        ...
        self.X_HIGH = SITE_WIDTH + (self.x_high - 1) * X_SITE_SEP + X_HIGH_PAD
    ...
    def update_init(self):
        ...
        self.qubit_scat = self.ax.scatter(
            [0 for _ in range(self.n_q)], [0 for _ in range(self.n_q)]
        )
        self.qubit_scat.set_color("b")
        self.col_plots = [
            self.ax.axvline(0, self.Y_LOW, self.Y_HIGH, c=(1, 0, 0, 0), ls="--")
            for _ in range(self.c_high)
        ]
        self.row_plots = [
            self.ax.axhline(0, self.X_LOW, self.X_HIGH, c=(1, 0, 0, 0), ls="--")
            for _ in range(self.c_high)
        ]
    ...
    def update_move(self, f: int, inst: dict, prev_state: dict):
        ...
        for col in inst["cols"]:
            curr_x = self.interpolate(progress, duration, col["begin"], col["end"])
            col_xs[col["id"]] = curr_x
            self.col_plots[col["id"]].set_xdata((col_xs[col["id"]],))

        row_ys = [row["y"] for row in prev_state["rows"]]
        for row in inst["rows"]:
            curr_y = self.interpolate(progress, duration, row["begin"], row["end"])
        ...
        self.qubit_scat.set_offsets([(q_xs[i], q_ys[i]) for i in range(self.n_q)])
        return
